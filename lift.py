...
def simulate_elevator_system(elevators, calls, num_floors):
    for call_location, call_direction, call_destination in calls:
        ...
        print(
            f"Request from floor {call_location} to floor {call_destination}: selected {selected_elevator.name}"
        )
...
#     threads = []
#     for elevator in elevators:
#         thread = threading.Thread(target=elevator.move, args=(num_floors,))
