        ...
        while self.requests:
            ...
            if self.direction == 'up':
                ...
                if next_requests:
                    ...
                    print(f"Elevator {self.name} reached floor {self.location} next request {next_request}")

                    # Handle destination floor
                    if next_request[2] is not None:
                        print(f"Elevator {self.name} picked up request to floor {next_request[2]}")
...
def simulate_elevator_system(elevators, calls, num_floors):
    ...
    for elevator in elevators:
        if len(elevator.requests) > 0:
            print(f"Elevator {elevator.name} is moving with requests {elevator.requests}")
